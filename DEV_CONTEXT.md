...
**Version 4.3 · April 2026 · BD/Partnerships Edition**
...
├── BULLET_LIBRARY.md                   ← Resume bullet raw text - Load into Claude
...
The extension can sync both "Outreach Contacts" and "Pipeline Roles".
- **`syncOutreach`**: Sends message to `background.js`, which adds to the `jobos_outreach_sync` storage key (contacts array).
- **`syncPipeline`**: Sends message to `background.js`, which adds to the `jobos_outreach_sync` storage key (pipeline array).
- **Export**: User goes to the extension Nudges tab and clicks "Export scored roles as JSON" - this copies the JSON payload to clipboard.
- **Import**: User pastes JSON into the Dashboard `/sync` panel and clicks Import. The `importFromExtension()` function deduplicates by company/role, saves to `jobos_outreach_v1` and `jobos_v3`, then immediately calls `renderOutreachTable()`, `updateOutreachMetrics()`, and `renderPipelineMini()` to refresh all affected views.
...
*Last updated: April 13, 2026 · Job Search OS v4.3 · file: index.html*
