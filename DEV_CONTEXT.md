...
## Architecture overview (v4.2 Optimized)
...
2. **`BULLET_LIBRARY.md`**: Raw text for all bullets. Only read during resume/application tasks.
3. **`WORKFLOW_GUIDE.md`**: Detailed logic for slash commands. Allows the dashboard to send minified instructions.

### Structural Prompting (XML)
Every prompt generated by `index.html` or `popup.js` now uses XML tags (e.g., `<role>`, `<jd>`, `<pipeline>`). This improves Claude's parsing accuracy and allows for cleaner prompt construction.

### Extension-to-Dashboard Sync
The extension can sync both "Outreach Contacts" and "Pipeline Roles".
- **`syncOutreach`**: Adds to the `jobos_outreach_sync` storage key (contacts array).
- **`syncPipeline`**: Adds to the `jobos_outreach_sync` storage key (pipeline array).
- **Import**: Dashboard `/sync` panel imports both, deduplicating by company/role and saving to `jobos_outreach_v1` and `jobos_v3` respectively.
